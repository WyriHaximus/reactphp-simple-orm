...
use EventSauce\ObjectHydrator\PropertyTypeResolver;
use LogicException;
use ReflectionClass;
...
class NaivePropertyTypeResolver implements PropertyTypeResolver
{
    public function typeFromConstructorParameter(
        ...
        if (! $type instanceof ReflectionNamedType || ($type->getName() !== 'array' && $type->getName() !== 'iterable')) {
    ...
    private function resolveFromConstructorDocComment(
        ...
        foreach ($matches as [, $type, $paramName]) {
            ...
            if (in_array(ltrim($type, '\\'), ['bool', 'boolean', 'int', 'integer', 'float', 'double', 'string', 'array', 'object', 'null', 'mixed', 'iterable'])) {
                continue;
            }
            ...
            $parameterType = array_key_exists($base, $useMap) ? $useMap[$base] : ltrim($namespace . '\\' . $base, '\\');
        }
    ...
    private function extractItemType(string $type): string
    {
        ...
        if (preg_match('/(?:array|list|iterable)<(?:(int(?:eger)?|string|mixed),\s*)?([A-Za-z0-9\\\]+)>/', $type, $matches)) {
            return $matches[2];
        }
